...
document.addEventListener("click", function (event) {
    ...
    if (event.target.hasAttribute("data-card")) {
        ...
        // Работа с корзиной для мобилки
        let basketHide = basket.querySelector(".basket__show");
        ...
        if (itemInBasket) {
            ...
            // Работа с корзиной для мобилки
            if (basketItems.children.length > 1 && basketHide.classList.contains("basket-hide")) {
...
document.addEventListener("click", (event) => {
    ...
    if (event.target.closest(".basket__info")) {

        let basket = event.target.closest(".basket");

        let basketItems = basket.querySelector(".basket__items");

        let basketHide = basket.querySelector(".basket__show");


        basketHide.classList.toggle("basket-hide");

        if (basketHide.classList.contains("basket-hide") && basketItems.children.length > 1) {
            basket.classList.remove("basket-active");
        } else {
            basket.classList.add("basket-active");
        }
